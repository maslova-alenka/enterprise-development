    ...
    : CrudControllerBase<DoctorDto, DoctorCreateUpdateDto, int>(crudService, logger)
{
    ...
    public async Task<ActionResult<IList<AppointmentDto>>> GetDoctorAppointments(int id)
    {
        ...
        try
        {
            var res = await crudService.GetDoctorAppointments(id);
    ...
    [ProducesResponseType(204)]
    [ProducesResponseType(500)]
    public async Task<ActionResult<IList<PatientDto>>> GetDoctorPatients(int id)
    {
        logger.LogInformation("{method} method of {controller} is called with {id} parameter", nameof(GetDoctorPatients), GetType().Name, id);
        try
        {
            var res = await crudService.GetDoctorPatients(id);
